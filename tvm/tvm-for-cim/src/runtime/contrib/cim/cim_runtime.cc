...
/*!
 * \file src/runtime/contrib/arm_compute_lib/cim_runtime.cc
...
class CIMRuntime : public JSONRuntimeBase {
 public:
  ...
  /*!
   * \brief Initialize runtime. Create CIM layer from JSON
   * representation.
   *
  ...
  void Run() override {
    for (size_t i = 0; i < input_nodes_.size(); ++i) {
      ...
      if (nodes_[nid].GetOpType() == "input") {
        ...
        CheckCIMError(layer_.inputs[i].allocator()->import_memory(data));
      }
    }
    ...
    for (size_t i = 0; i < outputs_.size(); ++i) {
      ...
      void* data = data_entry_[eid]->data;
      CheckCIMError(layer_.outputs[i].allocator()->import_memory(data));
    }
 ...
 private:
  /*!
   * \brief Build CIM layer from JSON representation and cache.
   *
   * \note For the time being only one layer or operator is supported
   * per engine.
   */
  void BuildEngine() {
    std::shared_ptr<arm_compute::MemoryManagerOnDemand> mm = MakeCIMMemoryManager();
    int num_pools = 0;
  ...
  /*!
   * \brief CIM objects we cache in order to avoid needing to construct
  ...
  /*!
   * \brief Create an CIM tensor given the JSON representation. If scale
   * and offset are given, then create a quantized CIM tensor.
   *
   ...
   * \return CIM Tensor.
   */
  arm_compute::Tensor MakeCIMTensorFromJSONEntry(const JSONGraphNodeEntry& tensor,
    ...
    return MakeCIMTensorFromJSONNode(node, scale, offset, node_data);
  }
   ...
   * \return CIM Tensor.
   */
  arm_compute::Tensor MakeCIMTensorFromJSONNode(const JSONGraphNode& node,
                                                JSONGraphNodeEntry* scale = nullptr,
                                                JSONGraphNodeEntry* offset = nullptr,
                                                void* data = nullptr) {
    ...
    return MakeCIMTensor(node, data, scale_data, offset_data);
  }

  /*!
   * \brief Create a 2D convolution layer.
   *
   * \param layer The CIM layer to build. Containing inputs, outputs and the CIM function.
   * \param node The JSON representation of the operator.
   * \param mm The CIM conv2d layer can request auxiliary memory from TVM.
   */
  void CreateConvolution2DLayer(CachedLayer* layer, const JSONGraphNode& node,
    ...
    arm_compute::PadStrideInfo pad_stride_info = MakeCIMPadStride(padding, strides);
    ...
    if (node.GetOpName() == "qnn.conv2d") {
      ...
      layer->inputs.push_back(MakeCIMTensorFromJSONEntry(inputs[0], &inputs[4], &inputs[2]));
      layer->inputs.push_back(MakeCIMTensorFromJSONEntry(inputs[1], &inputs[5], &inputs[3]));
      if (has_bias) {
        layer->inputs.push_back(MakeCIMTensorFromJSONEntry(inputs[6]));
      }
      layer->outputs.push_back(
          MakeCIMTensorFromJSONNode(node, &inputs[6 + has_bias], &inputs[7 + has_bias]));
    } else {
      ICHECK(num_inputs >= 2U && num_inputs <= 3U)
          << "Convolution requires 3 inputs with a bias, 2 inputs without.";
      has_bias = num_inputs == 3;
      for (const auto& i : inputs) {
        layer->inputs.push_back(MakeCIMTensorFromJSONEntry(i));
      }
      layer->outputs.push_back(MakeCIMTensorFromJSONNode(node));
    }
   ...
   * \param layer The CIM layer to build. Containing inputs, outputs and the CIM function.
   * \param node The JSON representation of the operator.
   * \param mm The CIM fully connected layer can request auxiliary memory from TVM.
   */
  ...
  void CreatePoolingLayer(CachedLayer* layer, const JSONGraphNode& node) {
    ...
    arm_compute::PadStrideInfo pad_stride_info = MakeCIMPadStride(padding, strides, ceil_mode);
    ...
    // CIM exclude pad option is inverse to Relays include pad option.
    ...
    layer->inputs.push_back(MakeCIMTensorFromJSONEntry(node.GetInputs()[0]));
    layer->outputs.push_back(MakeCIMTensorFromJSONNode(node));
  ...
  void CreateMaximumLayer(CachedLayer* layer, const JSONGraphNode& node) {
    layer->inputs.push_back(MakeCIMTensorFromJSONEntry(node.GetInputs()[0]));
    layer->inputs.push_back(MakeCIMTensorFromJSONEntry(node.GetInputs()[1]));
    layer->outputs.push_back(MakeCIMTensorFromJSONNode(node));
  ...
  void CreateAddLayer(CachedLayer* layer, const JSONGraphNode& node) {
    ...
    if ("add" == op_name) {
      ...
      layer->inputs.push_back(MakeCIMTensorFromJSONEntry(node.GetInputs()[0], &node.GetInputs()[2],
                                                         &node.GetInputs()[3]));
      layer->inputs.push_back(MakeCIMTensorFromJSONEntry(node.GetInputs()[1], &node.GetInputs()[4],
                                                         &node.GetInputs()[5]));
      layer->outputs.push_back(
          MakeCIMTensorFromJSONNode(node, &node.GetInputs()[6], &node.GetInputs()[7]));
    } else {
  ...
  /*! \brief Allow CIM functions to request auxiliary memory from TVM. */
  CIMAllocator allocator_;
  /*!
   * \brief The network layers represented by CIM functions.
  ...
  void Run() override {
    LOG(FATAL) << "Cannot call run on CIM module without runtime enabled. "
               << "Please build with USE_CIM_RUNTIME.";
  }
